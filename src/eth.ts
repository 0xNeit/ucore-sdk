...
function _ethJsonRpc(
  ...
  return new Promise<any>((resolve, reject) => {
    const provider = options._ucoreProvider || _createProvider(options);
 ...
 * @example
 * ```
 * const vBnbAddress = Ucore.util.getAddress(Ucore.vBNB);
 * 
 * (async function() {
 * 
 *   const srpb = await Ucore.eth.read(
 *     vBnbAddress,
 ...
 * 
 * (async function() {
 *   console.log('Supplying BNB to the Ucore Protocol...');
 * 
 *   // Mint some vBNB by supplying BNB to the Ucore Protocol
 *   const trx = await Ucore.eth.trx(
 *     vBnbAddress,
...
/**
 * This helps the Ucore.js constructor discover which Ethereum network the
 ...
 * (async function () {
 * 
 *   balance = await Ucore.eth.getBalance(myAddress, provider);
