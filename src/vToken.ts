...
export async function supply(
  ...
  // }
  if (vTokenName === constants.vCORE) {
    options.abi = abi.vCORE;
  } else {
    options.abi = abi.vERC20;
  }

  options._ucoreProvider = this._provider;

  if (vTokenName !== constants.vCORE && noApprove !== true) {
...
export async function redeem(
  ...
  const trxOptions: CallOptions = {
    ...
    abi: vTokenName === constants.vCORE ? abi.vCORE : abi.vERC20,
  };
...
export async function borrow(
  ...
  const parameters = [ amount ];
  trxOptions.abi = vTokenName === constants.vCORE ? abi.vCORE : abi.vERC20;
...
export async function repayBorrow(
  ...
  if (vTokenName === constants.vCORE) {
    ...
    trxOptions.abi = abi.vCORE;
  } else {
    parameters.push(amount);
    trxOptions.abi = abi.vERC20;
  }
